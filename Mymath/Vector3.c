...
/**********************************************************************************************************
*函 数 名: TransVelToFlightFrame
*功能说明: 修正视觉里程与机体坐标之间的速度
*形    参: 
*返 回 值: 无
**********************************************************************************************************/
void TransVelToFlightFrame(Vector3f_t velVIO,Vector3f_t* velFlight,float pitch){
	float sinPitch = sinf(pitch);
	float cosPitch = cosf(pitch);
	
	velFlight->x = velVIO.x * cosPitch - velVIO.z * sinPitch;
	velFlight->y = velVIO.y;
	velFlight->z = velVIO.x * sinPitch + velVIO.z * cosPitch;
